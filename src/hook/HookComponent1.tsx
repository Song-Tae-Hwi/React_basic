...
export default function HookComponent1() {
  ...
  // - mount : 컴포넌트가 처음 화면에 등록
  // - update : 컴포넌트 상태가 변경되어 리렐더링 되었을 때
  ...
  // 스코프할 상태 배열을 지정하지 않은 경우 모든 상태가 바뀔 때마다 실행됨
  // useEffect(() => {
  ...
  // 배열에 포함된 상태 중 하나라도 변경되면 effect 발생
  useEffect(() => {
...
function SubComponent() {
  ...
  // 전달한 콜백 함수의 return으로 컴포넌트가 umount 될 때 실행할 함수를 반환
  // 상태 스코프와 연결하여 사용할 필요가 없음
  useEffect(() => {
    console.log('sub component mount!');
    return () => {
      console.log('sub component umount !');
    };
  }, []);
