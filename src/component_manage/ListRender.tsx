...
export default function ListRender() {
  ...
  const fiEmployes = employees.filter(item => item.department === '재무');

  return (
    <div>
      {listItems}
      {counts.map(item => <h5 key={item}>반복작업 {item}번째</h5>)}
      {counts2.map((item, index) => <h5 key={index}>반복작업{item} {index}번째</h5>)}
      {fiEmployes.map((item, index) => <h5 key={index}>{`이름 : ${item.name} 부서 : ${item.department}`}</h5>)}
    </div>
  )
}
